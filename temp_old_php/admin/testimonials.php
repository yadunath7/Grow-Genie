...
if (isset($_POST['add'])) {
    ...
    $conn->query("INSERT INTO testimonials (name, role, content) VALUES ('$name', '$role', '$content')");
...
<head>
    ...
    <title>Testimonials Management | GrowGenie</title>
...
<body class="bg-white text-dark-black min-h-screen flex">
    ...
    <div class="w-64 h-screen sidebar fixed top-0 left-0 flex flex-col p-6 border-r-2 border-dark-black bg-white">
        ...
        <nav class="flex-1 space-y-2">
            ...
            <a href="testimonials.php" class="nav-item active flex items-center p-4 font-bold text-lg bg-lime-green border-2 border-dark-black rounded-2xl shadow-[4px_4px_0_#191a23]"><span>Testimonials</span></a>
    ...
    <main class="flex-1 ml-64 p-8 md:p-12">
        <header class="flex justify-between items-center mb-12">
            <div>
                <h2 class="text-5xl font-black uppercase">Testimonials</h2>
                <p class="text-xl text-gray-600 font-medium">Manage user feedback displayed on the site.</p>
            </div>
        </header>

        <!-- Add Form -->
        <div class="card-positivus mb-12">
            <h3 class="text-2xl font-black uppercase mb-6">Add New Testimonial</h3>
            <form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-black uppercase mb-2">User Name</label>
                ...
                <div>
                    <label class="block text-sm font-black uppercase mb-2">Role/Position</label>
                    <input type="text" name="role" required class="w-full bg-white border-2 border-dark-black rounded-2xl px-6 py-4 font-bold focus:shadow-[6px_6px_0_#b9ff66] focus:outline-none transition-all">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-black uppercase mb-2">Content</label>
                    <textarea name="content" required rows="3" class="w-full bg-white border-2 border-dark-black rounded-2xl px-6 py-4 font-bold focus:shadow-[6px_6px_0_#b9ff66] focus:outline-none transition-all"></textarea>
                </div>
                <div class="md:col-span-2">
                    <button type="submit" name="add" class="btn-positivus px-12">Save Testimonial</button>
                </div>
            </form>
        </div>
            ...
            <div class="card-positivus relative group">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <p class="text-lime-green font-bold text-sm uppercase"><?php echo htmlspecialchars($row['role']); ?></p>
                    </div>
                    <a href="?delete=<?php echo $row['id']; ?>" onclick="return confirm('Delete this?')" class="text-red-500 hover:text-red-700 transition-colors">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    </a>
                </div>
                <p class="text-gray-600 font-medium italic">"<?php echo htmlspecialchars($row['content']); ?>"</p>
            </div>
            <?php endwhile; ?>
        </div>
    </main>
